...
print("\nStudent tests: improved credibility signals")

international_org = score_url("https://example.int/report", use_llm=False)["score"]
regular_org = score_url("https://example.org/report", use_llm=False)["score"]

check(
    international_org > regular_org,
    f"a restricted .int source ({international_org:.2f}) outranks a generic .org source ({regular_org:.2f})",
)

preprint_with_doi = score_url(
    "https://www.biorxiv.org/content/10.1101/2026.01.01.123456",
    use_llm=False,
)["score"]

preprint_without_doi = score_url(
    "https://www.biorxiv.org/content/example-preprint",
    use_llm=False,
)["score"]

check(
    preprint_with_doi == preprint_without_doi,
    f"a DOI does not boost a known preprint ({preprint_with_doi:.2f} vs {preprint_without_doi:.2f})",
)

neutral_path = score_url(
    "https://unknown-source.info/health-report",
    use_llm=False,
)["score"]

sensational_path = score_url(
    "https://unknown-source.info/miracle-cure",
    use_llm=False,
)["score"]

check(
    sensational_path < neutral_path,
    f"sensational URL language ({sensational_path:.2f}) scores below neutral language ({neutral_path:.2f})",
)
