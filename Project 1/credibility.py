...
# Preprint repositories host scholarly manuscripts that have not necessarily
# completed peer review. We track this separately from domain reputation so
# the scorer can distinguish publication status from publisher reputation.
PREPRINT_DOMAINS = {
    "arxiv.org",
    "biorxiv.org",
}
...
def _crossref_scholarly_evidence(url: str) -> Optional[Dict[str, Any]]:
    """
    Query Crossref for independent evidence that an unknown URL is associated
    with a scholarly journal article. Retry once after a request failure, then
    fail safely by returning None. Accept metadata only when Crossref identifies
    a journal article and its primary resource matches the URL's domain.
    Return the journal metadata and evidence when these checks succeed.
    """
    ...
    for attempt in range(2):
        try:
            response = requests.get(
                ...
                params={"query.bibliographic": url, "rows": 1},
                timeout=3,
            )
    ...
    resource_domain = _normalize_domain(resource_url)
    ...
    same_domain = (
        ...
        or resource_domain.endswith("." + domain)
        or domain.endswith("." + resource_domain)
    )
    ...
    journal_name = container[0] if container else "a scholarly journal"

    return {
        "journal": journal_name,
        "issn": item.get("ISSN", []),
        "evidence": f"Crossref identifies this domain as hosting journal articles in {journal_name}",
}
...
def rule_based_signals(url: str) -> List[Signal]:
    ...
    if match:
        ...
        for tld, score in TLD_SCORES.items():
            ...
            signals.append(
             ...
             # For an unknown domain, look for independent scholarly evidence.
        scholarly_evidence = _crossref_scholarly_evidence(url)
    ...
    # Signal 5:    sensational wording can be a weak warning sign of low-quality
    # or promotional content. It is not proof that a claim is false, so each
    # phrase receives only a modest penalty rather than determining the score.
    for fragment, delta in SENSATIONAL_PATH_PENALTIES.items():
        if fragment in path:
            signals.append(
                Signal(
                    "sensational_path",
                    delta,
                    f"path contains sensational phrase '{fragment}'",
                )
            )

    # Signal 6:  a DOI can provide evidence that a work is formally registered,
    # but a DOI does not by itself imply peer review or high credibility.
    # Known preprint repositories are therefore excluded from this bonus.
    if re.search(r"/10\.\d{4,9}/", path) and not any(
        domain == preprint or domain.endswith("." + preprint)
        for preprint in PREPRINT_DOMAINS
    ):
        signals.append(
            Signal(
                "doi",
                0.10,
                "URL contains a DOI associated with a non-preprint scholarly source",
            )
        )

    # Signal 7: preprints are scholarly works but have not necessarily
    # completed peer review, so apply a modest credibility penalty.
    if any(domain == preprint or domain.endswith("." + preprint) for preprint in PREPRINT_DOMAINS):
        signals.append(
            Signal(
                "preprint",
                -0.10,
                "source is a preprint repository; the work may not have completed peer review",
            )
        )
...
def score_url(url: str, use_llm: Optional[bool] = None) -> Dict[str, Any]:
    ...
    band = "HIGH" if final >= 0.70 else "MEDIUM" if final >= 0.40 else "LOW"
    result = {
        "score": final,
        "explanation": f"{band} credibility ({final:.2f}). " + "; ".join(parts) + ".",
    }
